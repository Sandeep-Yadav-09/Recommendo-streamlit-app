...
# %%
    if page == "Dashboard":
        ...
        with col1:
            ...
            # Display merged dataframes   
            AgGrid(top_products_qty, theme='balham', height = 200, width = 150)
        with col3:
            html_temp_title = """
            <div style="background-color:#ABBAEA;padding:4px">
        ...
        with col1:
            ...
            total_quantity_sold = retail[retail['Country'] != 'United Kingdom'].groupby('Country')['Quantity'].sum().reset_index()

            # Plotting
            # fig = px.bar(sales_wo_uk, x ="Sales Revenue", y = "Country", color= 'Sales Revenue')
            fig = px.bar(total_quantity_sold, x ="Quantity", y = "Country", color= 'Quantity')
            fig.update_layout(showlegend=False,
                            height=400, width = 650,
                            margin={'l': 10, 'r': 10, 't': 0, 'b': 0})
            fig.update(layout_coloraxis_showscale=False)
